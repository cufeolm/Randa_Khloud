...
20- Swap word. reg-reg (based on SPARC-v8 ISA): enter --> SRwMas
21- Swap word. reg-imm (based on SPARC-v8 ISA): enter --> SRwM
22- Swap byte. (based on ARM-v8 ISA): enter --> Sabbram
23- Atomic load store reg-imm (based on SPARC-v8 ISA): enter --> ALUB
24- Atomic load store reg-reg (based on SPARC-v8 ISA): enter --> ALUBas
any other input will simulate no operation or make an error in the simulation
DUT: """;
		z=raw_input(s)
		if z == "1":
